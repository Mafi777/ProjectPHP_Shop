...
if (isset($_POST['dostavchik_id'])) {
    $dostavchik_id = $_POST['dostavchik_id'];

    
    $sql = "SELECT dp.id, p.name AS product_name, g.name AS group_name, dp.delivery_price, dp.quantity, dp.delivery_date
    FROM Dostavki_Product dp
    JOIN Products p ON dp.product_id = p.id
    JOIN `Group` g ON dp.group_id = g.id
    JOIN Dostavchik d ON dp.dostavchik_id = d.id
    WHERE d.id = '$dostavchik_id'";
    ...
    if ($result->num_rows > 0) {
        echo "<h2>Информация за доставки от доставчик с ID: $dostavchik_id</h2>";
        echo "<table>";
        echo "<tr><th>ID</th><th>Продукт</th><th>Група</th><th>Доставна цена</th><th>Брой</th><th>Дата на доставка</th></tr>";
        ...
        echo "Няма налични данни за доставки от доставчик с ID: $dostavchik_id.";
    }
}
